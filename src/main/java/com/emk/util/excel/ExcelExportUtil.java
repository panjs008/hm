...
import com.emk.util.excel.annotation.ExcelCollection;
import com.emk.util.excel.annotation.ExcelTarget;
import com.emk.util.excel.entity.ComparatorExcelField;
import com.emk.util.excel.entity.ExcelExportEntity;
import com.emk.util.excel.entity.ExcelTitle;
import com.emk.util.excel.entity.TemplateExportParams;
...
public final class ExcelExportUtil {
	...
	public static void createSheetInUserModel2File(HSSFWorkbook workbook,
		...
		try {
			...
			if (entity.getTitle() != null) {
				...
				sheet.createFreezePane(0, 1+i, 0, 1+i);
				index += i;
			} else {
		...
		for (int i = 0, exportFieldTitleSize = excelParams.size(); i < exportFieldTitleSize; i++) {
			...
			if (entity.getList() != null) {
				...
				/*sheet.addMergedRegion(new CellRangeAddress(index, index+1, cellIndex,
						cellIndex));  index+n   就是标题合并 index+n行    */
	 ...
	 * @param entity 
	 */
	public static void createStringCell(Row row, int index, String text,
	...
	private static int createHeaderRow(ExcelTitle entity, Sheet sheet,
		...
		if(entity.getSecondTitle()!=null){
	        ...
	        String temp_str=sdf.format(date1);
			createStringCell(row, 0, entity.getSecondTitle()+ " 导出时间："+temp_str, style,null);
			sheet.addMergedRegion(new CellRangeAddress(1, 1, 0, feildWidth));
			/**导出时间**/
//			 Date date1 = new Date();//当前时间
//		     SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");  
//		    String temp_str=sdf.format(date1);
//			createStringCell(row, 0,, style,null);
//			sheet.addMergedRegion(new CellRangeAddress(1, 1, 0, feildWidth));
			return 2;
		}
		return 1;
