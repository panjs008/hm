...
public class FlowController {
    ...
    public ModelAndView goProcess(String processUrl, HttpServletRequest req) {
        ...
        if(processUrl.equals("com/emk/storage/enquiry/emkEnquiry-process")){
            ...
            List<EmkPriceEntity> priceEntityList = systemService.findHql("from EmkPriceEntity where xpNo=? order by kdDate asc",enquiryEntity.getEnquiryNo());
            req.setAttribute("priceEntityList",priceEntityList);
            List<EmkSampleRequiredEntity> sampleRequiredEntityList = systemService.findHql("from EmkSampleRequiredEntity where enquiryNo=? order by kdDate asc",enquiryEntity.getEnquiryNo());
            req.setAttribute("sampleRequiredEntityList",sampleRequiredEntityList);
...
//        EmkContractEntity t = systemService.get(EmkContractEntity.class, id);
    ...
    public void deployFlow(HttpServletResponse response) throws Exception {
        ...
        //获取在classpath下的流程文件
        InputStream in = this.getClass().getClassLoader().getResourceAsStream("/process/sample.zip");
