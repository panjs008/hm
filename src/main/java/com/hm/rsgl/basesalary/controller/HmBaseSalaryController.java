...
public class HmBaseSalaryController extends BaseController {
	...
	@ResponseBody
	public AjaxJson doAddChange(HmChangeSalaryEntity hmBaseSalary, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "薪酬调薪添加成功";
		try{
			HmBaseSalaryEntity baseSalaryEntity = systemService.findUniqueByProperty(HmBaseSalaryEntity.class,"realName",hmBaseSalary.getRealName());
			MyBeanUtils.copyBeanNotNull2Bean(hmBaseSalary,baseSalaryEntity);
	...
	@ResponseBody
	public AjaxJson doUpdateChange(HmChangeSalaryEntity hmBaseSalary, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "薪酬调薪更新成功";
		HmChangeSalaryEntity t = hmBaseSalaryService.get(HmChangeSalaryEntity.class, hmBaseSalary.getId());
		HmBaseSalaryEntity baseSalaryEntity = systemService.findUniqueByProperty(HmBaseSalaryEntity.class,"realName",hmBaseSalary.getRealName());
	...
	public ModelAndView goAddChange(HmChangeSalaryEntity hmBaseSalary, HttpServletRequest req) {
		req.setAttribute("kdDate", DateUtil.format(new Date(), "yyyy-MM-dd"));

		List<Map<String, Object>> categoryEntities = this.systemService.findForJdbc("select lower(code) code,queryed,name,lower(PARENT_CODE) parentCode,required,column_type from hm_category where PARENT_CODE='A01A02' order by order_num asc");
		req.setAttribute("categoryEntities",categoryEntities);
		if (StringUtil.isNotEmpty(hmBaseSalary.getRealName())) {
			HmBaseSalaryEntity t = hmBaseSalaryService.findUniqueByProperty(HmBaseSalaryEntity.class,"realName", hmBaseSalary.getRealName());
	...
	public String exportXls(HmBaseSalaryEntity hmBaseSalary,HttpServletRequest request,HttpServletResponse response
		...
		sql.append(" select t1.* from hm_base_salary t1 ");
	...
	public Object importExcel(HttpServletRequest request, HttpServletResponse response) {
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				...
				for(int z = 3; z < itemValue0.size() ; z++){
				...
				for (int i = 1; i <= sheet.getLastRowNum(); i++) {
					...
					if(Utils.notEmpty(itemValue.get(0))){
						...
						hmBaseSalaryEntity.setRealName(itemValue.get(0));
						hmBaseSalaryEntity.setDeptName(itemValue.get(1));
						hmBaseSalaryEntity.setWorkName(itemValue.get(2));
						this.systemService.executeSql("delete from hm_base_salary where real_name=?",hmBaseSalaryEntity.getRealName());
...
//						Map org = systemService.findOneForJdbc("select org_code from t_s_depart where departname=? and parent_org_code='A01' limit 0,1",hmBaseSalaryEntity.getDeptName());
//						if(Utils.notEmpty(org)){
//							hmBaseSalaryEntity.setDeptCode(org.get("org_code").toString());
//							org = systemService.findOneForJdbc("select org_code from t_s_depart where departname=? and parent_org_code=? limit 0,1",hmBaseSalaryEntity.getWorkName(),hmBaseSalaryEntity.getDeptCode());
//							if(Utils.notEmpty(org)){
//								hmBaseSalaryEntity.setWorkCode(org.get("org_code").toString());
//							}
//						}

						this.systemService.save(hmBaseSalaryEntity);
						String sql = "update hm_base_salary set  ";
						int  codei = 0;

						for(int z = 0 ; z < codeList.size() ; z++){
							sql += codeList.get(z)+"='"+itemValue.get(z+3)+"',";
						}
						if(itemValue.size()>1){
