...
public class HmStaffController extends BaseController {
	 ...
	 * @param user
	 */
	@RequestMapping(params = "datagridOld")
	public void datagridOld(HmStaffEntityForOld hmStaff,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityForOld.class, dataGrid);
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		//查询条件组装器
		...
		if(Utils.isEmpty(param.get("month"))){
			param.put("month","0");
		}
		...
		try{
			//自定义追加查询条件
			cq.add(Restrictions.sqlRestriction("({alias}.rz_date is not null and {alias}.rz_date !='')"));

			cq.add(Restrictions.sqlRestriction("(left({alias}.rz_date,7)='"+oneYearBefore.substring(0,7)+"' or left({alias}.rz_date,7)='"+threeYearBefore.substring(0,7)+"' or left({alias}.rz_date,7)='"+fiveYearBefore.substring(0,7)+"' )"));

		/*	if(param.get("month").equals("0")){
				cq.add(Restrictions.sqlRestriction("(left({alias}.rz_date,7)='"+oneYearBefore.substring(0,7)+"')"));
			}
			if(param.get("month").equals("1")){
				cq.add(Restrictions.sqlRestriction("(left({alias}.rz_date,7)='"+threeYearBefore.substring(0,7)+"' and (lyg_state='1' or lyg_state is null))"));
//				cq.add(Restrictions.sqlRestriction("({alias}.rz_date>='"+fiveYearBefore+"' and {alias}.rz_date<'"+threeYearBefore+"')"));
			}
			if(param.get("month").equals("2")){
				cq.add(Restrictions.sqlRestriction("(left({alias}.rz_date,7)='"+fiveYearBefore.substring(0,7)+"' and (lyg_state='2' or lyg_state is null))"));
//				cq.add(Restrictions.sqlRestriction("({alias}.rz_date<'"+fiveYearBefore+"' )"));
	...
	@RequestMapping(params = "datagridC")
	public void datagridC(HmStaffEntityC hmStaff, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityC.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, hmStaff, request.getParameterMap());
		try{
			if(Utils.notEmpty(param.get("orgCode")) &&  !"A01".equals(param.get("orgCode"))){
	...
	@RequestMapping(params = "datagridD")
	public void datagridD(HmStaffEntityD hmStaff,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityD.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, hmStaff, request.getParameterMap());
		try{
			if(Utils.notEmpty(param.get("orgCode")) &&  !"A01".equals(param.get("orgCode"))){
	...
	@RequestMapping(params = "datagridE")
	public void datagridE(HmStaffEntityE hmStaff, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityE.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, hmStaff, request.getParameterMap());
		try{
			if(Utils.notEmpty(param.get("orgCode")) &&  !"A01".equals(param.get("orgCode"))){
	...
	@RequestMapping(params = "datagridF")
	public void datagridF(HmStaffEntityF hmStaff, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityF.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, hmStaff, request.getParameterMap());
		try{
			if(Utils.notEmpty(param.get("orgCode")) &&  !"A01".equals(param.get("orgCode"))){
	...
	@RequestMapping(params = "datagridG")
	public void datagridC(HmStaffEntityG hmStaff, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityG.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, hmStaff, request.getParameterMap());
		try{
			if(Utils.notEmpty(param.get("orgCode")) &&  !"A01".equals(param.get("orgCode"))){
	...
	public AjaxJson doAdd(HmStaffEntity hmStaff, HttpServletRequest request) {
		...
		try{
			...
			hmStaff.setState("0");
			hmStaff.setLygState("0");

			tsDepart = systemService.findUniqueByProperty(TSDepart.class,"orgCode",hmStaff.getWorkCode());
			hmStaff.setWorkName(tsDepart.getDepartname());
			tsDepart = systemService.findUniqueByProperty(TSDepart.class,"orgCode",hmStaff.getGroupCode());
			hmStaff.setGroupName(tsDepart.getDepartname());
	...
	@ResponseBody
	public AjaxJson doApproval(HmStaffEntity hmStaff,String isPass, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
		message = "人员信息表更新成功";
		if("0".equals(isPass)){
			HmStaffEntity t = hmStaffService.get(HmStaffEntity.class, hmStaff.getId());
			try {
				Map<String, Object> category= this.systemService.findOneForJdbc("select lower(code) code,queryed,name,lower(PARENT_CODE) parentCode,required,column_type from hm_category where PARENT_CODE='A01A02' and name='老员工奖' order by order_num asc");
				HmBaseSalaryEntity baseSalaryEntity = systemService.findUniqueByProperty(HmBaseSalaryEntity.class,"realName",t.getRealName());
				Class c = Class.forName(HmBaseSalaryEntity.class.getName());
				String m = "setA"+category.get("code").toString().substring(1);
				Method show = c.getMethod(m,String.class);

				if(Utils.isEmpty(baseSalaryEntity)){
					baseSalaryEntity = new HmBaseSalaryEntity();
					MyBeanUtils.copyBeanNotNull2Bean(t,baseSalaryEntity);
					baseSalaryEntity.setId(null);
				}
				if("0".equals(hmStaff.getLygState())){
					Object object = show.invoke(baseSalaryEntity,"100");
					hmStaff.setLygState("1");
				}else if("1".equals(hmStaff.getLygState())){
					Object object = show.invoke(baseSalaryEntity,"200");
					hmStaff.setLygState("2");
				}else if("2".equals(hmStaff.getLygState())){
					Object object = show.invoke(baseSalaryEntity,"200");
					hmStaff.setLygState("3");
				}
				systemService.saveOrUpdate(baseSalaryEntity);
	...
	@RequestMapping(params = "getDept", method = {org.springframework.web.bind.annotation.RequestMethod.POST})
	@ResponseBody
	...
	public ModelAndView goAdd(HmStaffEntity hmStaff, HttpServletRequest req) {
		Map orderNum = systemService.findOneForJdbc("select CAST(ifnull(max(right(work_no,4)),0)+1 AS signed) orderNum from hm_staff");
		req.setAttribute("workNo",String.format("%04d", Integer.parseInt(orderNum.get("orderNum").toString())));
	...
	public String exportXls(HmStaffEntityB hmStaff, HttpServletRequest request, HttpServletResponse response
			, DataGrid dataGrid, ModelMap modelMap) {
		CriteriaQuery cq = new CriteriaQuery(HmStaffEntityB.class, dataGrid);
		Map<String,String> param = ParameterUtil.getParamMaps(request.getParameterMap());
		...
		List<HmStaffEntityB> hmStaffs = this.hmStaffService.getListByCriteriaQuery(cq,false);
		modelMap.put(NormalExcelConstants.FILE_NAME,"人员信息表");
		modelMap.put(NormalExcelConstants.CLASS,HmStaffEntityB.class);
				...
				for (int i = 1; i <= sheet.getLastRowNum(); i++) {
					...
					if(Utils.notEmpty(itemValue.get(0))){
						...
						if(Utils.notEmpty(staff)){
							...
							hmStaffEntity.setWorkNo(String.format("%04d", Integer.parseInt(orderNum.get("orderNum").toString())));
						}

						hmStaffEntity.setRealName(itemValue.get(0));
						hmStaffEntity.setDeptName(itemValue.get(1));
						hmStaffEntity.setWorkName(itemValue.get(2));
						hmStaffEntity.setGroupName(itemValue.get(3));
						...
						if(Utils.notEmpty(org)){
							...
							org = systemService.findOneForJdbc("select org_code from t_s_depart where departname=? and parent_org_code=? limit 0,1",hmStaffEntity.getWorkName(),hmStaffEntity.getDeptCode());
							if(Utils.notEmpty(org)){
								hmStaffEntity.setWorkCode(org.get("org_code").toString());
								org = systemService.findOneForJdbc("select org_code from t_s_depart where departname=? and parent_org_code=? limit 0,1",hmStaffEntity.getGroupName(),hmStaffEntity.getWorkCode());
								if(Utils.notEmpty(org)){
									hmStaffEntity.setGroupCode(org.get("org_code").toString());
								}else{
									tsDepart = new TSDepart();
									saveDept(tsDepart,"orgCode",hmStaffEntity.getWorkCode(),hmStaffEntity.getGroupName());
									hmStaffEntity.setGroupCode(tsDepart.getOrgCode());
								}
							}else{
								tsDepart = new TSDepart();
								saveDept(tsDepart,"orgCode",hmStaffEntity.getDeptCode(),hmStaffEntity.getWorkName());
								hmStaffEntity.setWorkCode(tsDepart.getOrgCode());
							...
							hmStaffEntity.setDeptCode(tsDepart.getOrgCode());
						...
						Map dict = systemService.findOneForJdbc("select t2.typecode from t_s_type t2 left join t_s_typegroup t1 on t1.ID=t2.typegroupid where t1.typegroupcode='job' and t2.typename=?  limit 0,1",itemValue.get(4));
						if(Utils.notEmpty(dict)){
							hmStaffEntity.setJob(dict.get("typecode").toString());
						}
						dict = systemService.findOneForJdbc("select t2.typecode from t_s_type t2 left join t_s_typegroup t1 on t1.ID=t2.typegroupid where t1.typegroupcode='jb' and t2.typename=?  limit 0,1",itemValue.get(5));
						if(Utils.notEmpty(dict)){
							hmStaffEntity.setJb(dict.get("typecode").toString());
						}
						dict = systemService.findOneForJdbc("select t2.typecode from t_s_type t2 left join t_s_typegroup t1 on t1.ID=t2.typegroupid where t1.typegroupcode='xclb' and t2.typename=?  limit 0,1",itemValue.get(6));
						if(Utils.notEmpty(dict)){
							hmStaffEntity.setXclb(dict.get("typecode").toString());
						}
						hmStaffEntity.setSex(itemValue.get(7));

						dict = systemService.findOneForJdbc("select t2.typecode from t_s_type t2 left join t_s_typegroup t1 on t1.ID=t2.typegroupid where t1.typegroupcode='yglb' and t2.typename=?  limit 0,1",itemValue.get(8));
						if(Utils.notEmpty(dict)){
							hmStaffEntity.setYglb(dict.get("typecode").toString());
						}

						dict = systemService.findOneForJdbc("select t2.typecode from t_s_type t2 left join t_s_typegroup t1 on t1.ID=t2.typegroupid where t1.typegroupcode='dg' and t2.typename=?  limit 0,1",itemValue.get(9));
						if(Utils.notEmpty(dict)){
							hmStaffEntity.setTaker(dict.get("typecode").toString());
						}

						if("固定".equals(itemValue.get(10))){
							hmStaffEntity.setWorkType("0");
						}
						if("临时".equals(itemValue.get(10))){
							hmStaffEntity.setWorkType("1");
						}
						...
						dict = systemService.findOneForJdbc("select t2.typecode from t_s_type t2 left join t_s_typegroup t1 on t1.ID=t2.typegroupid where t1.typegroupcode='xueli' and t2.typename=?  limit 0,1",itemValue.get(20));
						if(Utils.notEmpty(dict)){
							hmStaffEntity.setXueli(dict.get("typecode").toString());
						}
						hmStaffEntity.setHousePhone(itemValue.get(21));
						hmStaffEntity.setJjlxr(itemValue.get(22));
						if("是".equals(itemValue.get(23))){
							hmStaffEntity.setIsNs("0");
						}
						if("否".equals(itemValue.get(24))){
							hmStaffEntity.setIsNs("1");
						}
						hmStaffEntity.setHjszd(itemValue.get(25));
