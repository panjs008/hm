...
public class HmStaffEntityForOld implements java.io.Serializable {
	/**主键*/
	private String id;
	/**创建人名称*/
	...
	private String lygState;
	private String workYearV;
	...
	@Transient
	public String getWorkYearV() {
		int diffM = 0;
		try {
			if(Utils.notEmpty(rzDate)){
				Calendar cal1 = DateUtils.parseCalendar(rzDate,"yyyy-MM-dd");
				Date date = new Date();//获取当前时间
				Calendar cal2 = Calendar.getInstance();
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				cal2.setTime(date);
				diffM  = DateUtils.dateDiff('y',cal2,cal1);
				return String.valueOf(diffM);
	...
	public void setWorkYearV(String workYearV) {
		this.workYearV = workYearV;
	}
	...
	@Column(name ="lyg_state",nullable=true,length=32)
	public String getLygState() {
		return lygState;
	}

	public void setLygState(String lygState) {
		this.lygState = lygState;
	}

	@Formula("(select hb.a01a02a11 from hm_base_salary hb where hb.work_no = work_no limit 0,1)")
	...
	//@Formula("(select TIMESTAMPDIFF(YEAR,hs.rz_date,DATE_FORMAT(NOW(), '%Y-%m-%d %H:%i:%S')) from hm_staff hs where hs.work_no = work_no limit 0,1)")
