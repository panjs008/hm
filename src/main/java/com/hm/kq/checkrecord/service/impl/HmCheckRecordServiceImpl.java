package com.hm.kq.checkrecord.service.impl;
...
@Service("hmCheckRecordService")
@Transactional
public class HmCheckRecordServiceImpl extends CommonServiceImpl implements HmCheckRecordServiceI {
	 ...
	 * @return
	 */
	private void doAddBus(HmCheckRecordEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doUpdateBus(HmCheckRecordEntity t) throws Exception{
		//-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
	 ...
	 * @return
	 */
	private void doDelBus(HmCheckRecordEntity t) throws Exception{
	    //-----------------sql增强 start----------------------------
	 	//-----------------sql增强 end------------------------------
 	...
 	private Map<String,Object> populationMap(HmCheckRecordEntity t){
		...
		map.put("clock_time", t.getClockTime());
		map.put("kd_time", t.getKdTime());
		map.put("clock_result", t.getClockResult());
		return map;
	}
	 ...
	 * @return
	 */
 	public String replaceVal(String sql,HmCheckRecordEntity t){
 		...
 		sql  = sql.replace("#{clock_time}",String.valueOf(t.getClockTime()));
 		sql  = sql.replace("#{kd_time}",String.valueOf(t.getKdTime()));
 		sql  = sql.replace("#{clock_result}",String.valueOf(t.getClockResult()));
 		...
 		if(StringUtil.isNotEmpty(cgJavaValue)){
			...
			try {
				...
				if(obj instanceof CgformEnhanceJavaInter){
					...
					javaInter.execute("hm_check_record",data);
				}
			} catch (Exception e) {
				e.printStackTrace();
