...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			$("#workTimeDetailId").load("hmOrderController.do?workTimeList&workId=${hmOrderPage.id }");
			$("#detailId").load("hmWorkController.do?detailList&workId=${hmOrderPage.id }");
		...
		function addWorkTime(){
			...
			for(var i = 0 ; i < gdNamesCodeArr.length ; i++){
				$('#workNo'+(flagWT-gdNamesCodeArr.length+i+1)).val(gdNamesCodeArr[i]);
				$('#realName'+(flagWT-gdNamesCodeArr.length+i+1)).val(gdNamesArr[i]);
				$('#sex'+(flagWT-gdNamesCodeArr.length+i+1)).val(sexsArr[i]);
				if(jobArr[i] != null){
					if(jobArr[i].indexOf('组长') >0 && jobArr[i].indexOf('副组长')<0){
						$('#hjV'+(flagWT-gdNamesCodeArr.length+i+1)).val('2.2');
						$('#zz'+(flagWT-gdNamesCodeArr.length+i+1)).val('1');

					}else{
						$('#hjV'+(flagWT-gdNamesCodeArr.length+i+1)).val('1.05');
					}
				}

				$('#type'+(flagWT-gdNamesCodeArr.length+i+1)).val(type);
				$('#yglb'+(flagWT-gdNamesCodeArr.length+i+1)).val(yglbArr[i]);
			}*/
...
<body>
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="hmOrderController.do?doAdd" tiptype="1">
