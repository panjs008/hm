                ...
                success : function(data) {
                    ...
                    if (d.success) {
                        ...
                        for(var i=0;i<=indexFlag;i++){
                            result["storageSetId"+i] = $("#proFrm").contents().find("#storageSetId"+i).val();
                            result["storageSetPosiId"+i] = $("#proFrm").contents().find("#storageSetPosiId"+i).val();
                            result["proId"+i] = $("#proFrm").contents().find("#proId"+i).val();
                            result["signTotal"+i] = $("#proFrm").contents().find("#signTotal"+i).val();
                        }

                        $.ajax({
                            url:"emkInStorageDetailController.do?saveRows&inStorageId="+d.obj+"&dataSize="+indexFlag+"&data="+JSON.stringify(result),
                            data:null,
                            type:'post',
                            success:function(data){
...
<body>
<span style="text-align: left;float: left;font-size: 22px"><img src="images/order.jpg" style="float: left;width: 35px;height:35px;">&nbsp;<strong>入库单&nbsp;${CUR_USER.currentDepart.departname}&nbsp;${CUR_USER.realName}&nbsp;${inStorageEntity.htbz}&nbsp;<font color="red">${heji}</font>
	<c:if test="${inStorageEntity.state eq 0}"><font color="red">待入库</font></c:if>
	<c:if test="${inStorageEntity.state eq 1}"><font color="blue">已入库</font></c:if></strong>&nbsp;&nbsp;</span>
<div style="padding: 3px; height: 35px;width:auto;" class="datagrid-toolbar" align="right">
	<c:if test="${inStorageEntity.state eq 0}">
		<input class="btn" type="submit" value="保存" style="height:30px;width:100px !important;border-radius:5px" onclick="save();"></c:if>
...
<div class="easyui-tabs" style="height:475px;overflow:hidden" id="orderTabId">
	...
	<div id="proDiv" title="商品信息" style="padding:5px;height:440px;overflow:hidden" >
		<iframe scrolling="yes" id="proFrm"  frameborder="0"  src="emkInStorageDetailController.do?list&inStorageId=${inStorageEntity.id}&state=${inStorageEntity.state}" style="width:100%;height:100%;"></iframe>
	</div>
	<div id="companyDiv" title="公司抬头" style="height:500px;overflow:hidden" >
		<iframe scrolling="yes" id="companyFrm"  frameborder="0"  src="emkOrderCompanyTopController.do?goAdd2&id=${inStorageEntity.id}" style="width:100%;height:100%;"></iframe>
	</div>
